(function () {
  ...
  function buildPreviewHTML() {
    ...
    if (homepageData) {
      ...
      html += '<div class="preview-item"><span class="preview-label">Service:</span><span class="preview-value">' + escapeHtml(homepageData.service) + '</span></div>';
      html += '<div class="preview-item"><span class="preview-label">Budget:</span><span class="preview-value">' + escapeHtml(homepageData.budget) + '</span></div>';
      ...
      if (homepageData.timeline) {
        ...
        html += '<div class="preview-item"><span class="preview-label">Timeline:</span><span class="preview-value">' + escapeHtml(timelineLabels[homepageData.timeline] || homepageData.timeline) + '</span></div>';
      }
      html += '<div class="preview-item" style="grid-column:1/-1;"><span class="preview-label">Details:</span><span class="preview-value">' + escapeHtml(homepageData.details) + '</span></div>';
      html += '</div></div>';
    }
    ...
    html += '<div class="preview-item"><span class="preview-label">Reference:</span><span class="preview-value">' + escapeHtml(reference) + '</span></div>';
    html += '</div></div>';
  ...
  function buildWhatsAppMessage() {
    ...
    if (homepageData) {
      ...
      msg += 'Service: ' + encodeURIComponent(serviceLabels[homepageData.service] || homepageData.service) + '%0A';
      msg += 'Budget: ' + encodeURIComponent(homepageData.budget) + '%0A';
      msg += 'Company Size: ' + encodeURIComponent(companySizeLabels[homepageData.companySize] || homepageData.companySize) + '%0A';
      if (homepageData.timeline) {
        msg += 'Timeline: ' + encodeURIComponent(timelineLabels[homepageData.timeline] || homepageData.timeline) + '%0A';
      }
      msg += 'Details: ' + encodeURIComponent(homepageData.details) + '%0A%0A';
    } else {
  ...
  function initForm() {
    ...
    var autoSaveFields = ['reqName', 'reqEmail', 'reqPhone', 'reqBudget', 'reqReference', 'manualRequirements'];
  ...
  function renderHomepageSummary() {
    ...
    var budgetEl = document.getElementById('summaryBudget');
    var companySizeEl = document.getElementById('summaryCompanySize');
    var timelineEl = document.getElementById('summaryTimeline');
    ...
    if (serviceEl) serviceEl.textContent = homepageData.service || '';
    if (budgetEl) budgetEl.textContent = homepageData.budget || '';
    ...
    if (timelineEl) {
      ...
      timelineEl.textContent = timelineLabels[homepageData.timeline] || homepageData.timeline || '';
    }
    if (detailsEl) detailsEl.textContent = homepageData.details || '';
  }
